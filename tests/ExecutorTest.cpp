...
//测试跑车需求前进功能
TEST(ExecutorTest,sportscarM)
{
    std::unique_ptr<Executor> executor(Executor::NewExecutor({0, 0, 'N',1,2,1,0}));
...
//测试bus需求前进功能
TEST(ExecutorTest,busM)
{
    std::unique_ptr<Executor> executor(Executor::NewExecutor({0, 0, 'N',2,1,2,0}));
    executor->Execute("MMLMR");
...
{
    std::unique_ptr<Executor> executor(Executor::NewExecutor({0, 0, 'N',0,1,1,0}));
...
{
    std::unique_ptr<Executor> executor(Executor::NewExecutor({0, 0, 'E',0,1,1,1}));
...
{
    std::unique_ptr<Executor> executor(Executor::NewExecutor({0, 0, 'S',0,1,1,2}));
